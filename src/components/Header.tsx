          ...
          <div className="bg-white/95 backdrop-blur-md border-t border-gray-200 py-4 rounded-b-2xl shadow-xl mx-4 mb-4">
            <nav className="flex flex-col space-y-2 px-4">
              {[
                ...
                { href: '/farm', label: 'Farm', isActive: isActivePage('/farm') }
              ].map((item, index) => (
                <a
                  key={item.href}
                  href={item.href}
                ...
                >
                  ...
                  {/* Hover glow effect */}
                  <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-20 transition-opacity duration-300 blur-sm bg-green-300"></div>
                </a>
              ))}
            </nav>
          </div>
